...
const getContacts = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(contacts.map(contact => ({
    ...contact.toJSON(),
    imageUrl: `https://example.com/images/${contact.image}` // Assuming image URLs are stored in the database
  })));
});
...
const createContact = asyncHandler(async (req, res) => {
  ...
  const contact = await Contact.create({
    ...
    phone,
    user_id: req.user.id,
    image: req.body.image // Assuming the image link is passed in the request body
  });
